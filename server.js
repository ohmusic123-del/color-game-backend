...
async function initializeRoundSystem() {
    try {
        ...
        const existingRound = await Round.findOne({ winner: null })
            .sort({ createdAt: -1 });
        
        if (existingRound) {
            CURRENT_ROUND = {
                id: existingRound.roundId,
                startTime: existingRound.createdAt.getTime()
            };
            ...
            console.log(`🎮 Created new round: ${CURRENT_ROUND.id}`);
        }
    } catch (err) {
        console.error('❌ Round initialization failed:', err);
    }
}
...
app.post('/register', async (req, res) => {
    try {
        ...
        // Generate JWT token
        const token = jwt.sign({ mobile: newUser.mobile }, process.env.JWT_SECRET, { expiresIn: '30d' });
...
// FIXED BET ENDPOINT WITH TRANSACTIONS
...
app.post("/withdraw", auth, async (req, res) => {
    ...
    try {
        ...
        const requiredWager = (user.depositAmount || 0) * 1; // 1x wagering
        if (user.totalWagered < requiredWager) {
            await session.abortTransaction();
            return res.status(400).json({ 
                error: `Wager ₹${(requiredWager - user.totalWagered).toFixed(2)} more` 
            });
        }
        ...
        if (!user.withdrawMethod) {
            await session.abortTransaction();
            return res.status(400).json({ error: "Set withdrawal method first" });
        }
...
app.post("/admin/user/ban", adminAuth, async (req, res) => {
    try {
        const { mobile, banned } = req.body;
        await User.updateOne({ mobile }, { banned: banned === true });
...
app.post("/admin/withdraw/:id", adminAuth, async (req, res) => {
    try {
        ...
        if (action === "approve") {
            ...
            // Release held amount
            user.withdrawalHeld = Math.max(0, (user.withdrawalHeld || 0) - withdrawal.amount);
