...
let RAHUL_MODI_ROUND = { id: null, startTime: null };
...
async function getNextRahulModiRoundId() {
  const lastRound = await RahulModiRound.findOne().sort({ createdAt: -1 });
  if (!lastRound) return Date.now().toString();
  return (parseInt(lastRound.roundId) + 1).toString();
}
...
app.post("/rahulmodi/bet", auth, async (req, res) => {
  try {
    ...
    const bet = await RahulModiBet.create({
      mobile: user.mobile,
      roundId: RAHUL_MODI_ROUND.id,
      option,
      amount: betAmount,
    ...
    const round = await RahulModiRound.findOne({ roundId: RAHUL_MODI_ROUND.id });
    if (round) {
      if (option === 'rahul') {
        round.rahulPool = (round.rahulPool || 0) + betAmount;
      } else {
        round.modiPool = (round.modiPool || 0) + betAmount;
      }
      await round.save();
    }

    console.log(`🎮 Rahul Modi bet: ${user.mobile} - ${option.toUpperCase()} - ₹${betAmount}`);
...
app.get("/rahulmodi/bets", auth, async (req, res) => {
  try {
    ...
    console.error("Rahul Modi bets error:", err);
    res.status(500).json({ error: "Failed to load bets" });
  }
});
...
async function processRahulModiRoundEnd(roundId) {
  console.log(`\n🎮 PROCESSING RAHUL MODI ROUND: ${roundId}`);
  const session = await mongoose.startSession();
  ...
  try {
    ...
    console.log(`💰 RAHUL: ₹${rahulPool} | MODI: ₹${modiPool}`);
    ...
    await session.commitTransaction();
    session.endSession();
    console.log(`✅ Rahul Modi Round ${roundId} completed\n`);
    
  } catch (err) {
    console.error('❌ RAHUL MODI ROUND ERROR:', err);
    await session.abortTransaction();
    session.endSession();
  }
}
...
async function startNewRahulModiRound() {
  ...
  if (elapsed >= 60 && RAHUL_MODI_ROUND.id) {
    ...
    console.log(`✅ NEW RAHUL MODI ROUND: ${RAHUL_MODI_ROUND.id}`);
...
(async () => {
  try {
    ...
    // Initialize Rahul Modi Round
    const latestRMRound = await RahulModiRound.findOne().sort({ createdAt: -1 });
    
    if (latestRMRound && latestRMRound.roundId) {
      const roundAge = Date.now() - latestRMRound.createdAt.getTime();
      
      if (latestRMRound.winner === null && roundAge < 65000) {
        RAHUL_MODI_ROUND.id = latestRMRound.roundId;
        RAHUL_MODI_ROUND.startTime = latestRMRound.createdAt.getTime();
      } else {
